!================================================================================
module FF_Pair_LJ_Shift
  use Template_ForceField, only: ForceField
  use VarPrecision
  use Template_SimBox, only: SimBox
  use CoordinateTypes

  type, extends(forcefield) :: Pair_LJ_Shift
    ...
    real(dp), allocatable :: ShiftTable(:,:)
!    real(dp) :: rCut, rCutSq
    contains
      procedure, pass :: Constructor => Constructor_LJ_Shift
      procedure, pass :: DetailedECalc => Detailed_LJ_Shift
      procedure, pass :: DiffECalc => DiffECalc_LJ_Shift
      procedure, pass :: ShiftECalc_Single => Shift_LJ_Shift_Single
!      procedure, pass :: ShiftECalc_Multi => Shift_LJ_Shift_Multi
      procedure, pass :: NewECalc => New_LJ_Shift
      procedure, pass :: OldECalc => Old_LJ_Shift
      procedure, pass :: OrthoVolECalc => OrthoVol_LJ_Shift
      procedure, pass :: AtomExchange => AtomExchange_LJ_Shift
      procedure, pass :: ProcessIO => ProcessIO_LJ_Shift
      procedure, pass :: Prologue => Prologue_LJ_Shift
      procedure, pass :: GetCutOff => GetCutOff_LJ_Shift
  end type

  contains
  !=============================================================================+
  subroutine Constructor_LJ_Shift(self)
    ...
    allocate(self%ShiftTable(1:nAtomTypes, 1:nAtomTypes), stat = AllocateStat)

    self%eps = 4E0_dp
    self%sig = 1E0_dp
    self%rMin = 0.5E0_dp
    self%ShiftTable = 0E0_dp
  ...
  !===================================================================================
  subroutine Detailed_LJ_Shift(self, curbox, E_T, accept)
    ...
    rsq = self%rCutSq
    do atmType1 = 1, nAtomTypes
      do atmType2 = 1, nAtomTypes
        ep = self % epsTable(atmType2, atmType1)
        sig_sq = self % sigTable(atmType2, atmType1)
        LJ = (sig_sq/rsq)**3
        LJ = ep * LJ * (LJ-1E0_dp)
        self%ShiftTable(atmType2, atmType1) = LJ
        self%ShiftTable(atmType1, atmType2) = LJ
      enddo
    enddo
    ...
    do iAtom = 1, curbox%nMaxAtoms-1
      ...
      do jAtom = iAtom+1, curbox%nMaxAtoms
        ...
        if(rsq < self%rCutSq) then
          ...
          LJShift = self%ShiftTable(atmType1, atmType2)
          LJ = (sig_sq/rsq)**3
          LJ = ep * LJ * (LJ-1E0) - LJShift
          E_LJ = E_LJ + LJ
...
!============================================================================
  subroutine DiffECalc_LJ_Shift(self, curbox, disp, tempList, tempNNei, E_Diff, accept)
    implicit none
    class(Pair_LJ_Shift), intent(inout) :: self
    class(simBox), intent(inout) :: curbox
...
!         write(*,*) size(tempList)
      ...
      class default
        write(*,*) "Unknown Perturbation Type Encountered by the LJ_Shift Pair Style."
        stop
  ...
  !=====================================================================
  subroutine Shift_LJ_Shift_Single(self, curbox, disp, E_Diff, accept)
    implicit none
    class(Pair_LJ_Shift), intent(inout) :: self
    class(SimBox), intent(inout) :: curbox
...
!        write(*,*) sqrt(rsq)
        ...
        if(rsq < self%rCutSq) then
          ...
          LJShift = self%ShiftTable(atmType2, atmType1)
          LJ = (sig_sq/rsq)
          LJ = LJ * LJ * LJ
          LJ = ep * LJ * (LJ-1E0_dp) - LJShift
          E_Diff = E_Diff + LJ
...
!        write(*,*) sqrt(rsq)
  ...
  !=====================================================================
  subroutine New_LJ_Shift(self, curbox, disp, tempList, tempNNei, E_Diff, accept)
    implicit none
    class(Pair_LJ_Shift), intent(inout) :: self
    class(SimBox), intent(inout) :: curbox
...
!          write(*,*) iAtom, jAtom, rsq, LJ
  ...
  !=====================================================================
  subroutine Old_LJ_Shift(self, curbox, disp, E_Diff)
    implicit none
    class(Pair_LJ_Shift), intent(inout) :: self
    class(SimBox), intent(inout) :: curbox
...
!          write(*,*) iAtom, jAtom, rsq, LJ
  ...
  !=====================================================================
  subroutine OrthoVol_LJ_Shift(self, curbox, disp, E_Diff, accept)
    implicit none
    class(Pair_LJ_Shift), intent(inout) :: self
    class(SimBox), intent(inout) :: curbox
  ...
  !=====================================================================
  subroutine AtomExchange_LJ_Shift(self, curbox, disp, E_Diff, accept)
    implicit none
    class(Pair_LJ_Shift), intent(inout) :: self
    class(SimBox), intent(inout) :: curbox
...
!        rmin_ij = self % rMinTable(atmType1, atmType2)          
  ...
  !=====================================================================
  subroutine ProcessIO_LJ_Shift(self, line)
    use Common_MolInfo, only: nAtomTypes
...
!      call GetAllCommands(line, parlist, nPar, lineStat)
  ...
  !=============================================================================+
  function GetCutOff_LJ_Shift(self) result(rCut)
    implicit none
    class(Pair_LJ_Shift), intent(inout) :: self
    real(dp) :: rCut
  ...
  !=====================================================================
  subroutine Prologue_LJ_Shift(self)
    use Common_MolInfo, only: nAtomTypes
    use ParallelVar, only: nout
    implicit none
    class(Pair_LJ_Shift), intent(inout) :: self
