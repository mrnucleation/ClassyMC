...
!      procedure, pass :: GeneratePosition => EB_AVBMC_GeneratePosition
      ...
      procedure, pass :: SelectNeigh => EB_AVBMC_SelectNeigh
      procedure, pass :: SelectNeighReverse => EB_AVBMC_SelectNeighReverse
...
!    call MolData(molType) % molConstruct % ReverseConfig( trialBox, ProbSub, accept)
  ...
  modify analysis 4 neighlist 2    !Check Post Energy Constraint
...
!========================================================
  subroutine EB_AVBMC_SelectNeigh(self, trialBox, targetIndx, removeMolIndx, ProbOut)
    use Common_MolInfo, only: MolData, nMolTypes
...
!========================================================
  subroutine EB_AVBMC_SelectNeighReverse(self, trialBox, targetIndx, ProbOut)
    use Common_MolInfo, only: MolData, nMolTypes
    use RandomGen, only: ListRNG, grnd
    implicit none
    class(EB_AVBMC), intent(inout) :: self
...
!=========================================================================
  subroutine EB_AVBMC_Epilogue(self)
    ...
    write(nout,"(1x,A,I15)") "EB_AVBMC In Moves Accepted: ", nint(self%inaccpt)
    write(nout,"(1x,A,I15)") "EB_AVBMC In Moves Attempted: ", nint(self%inatmps)
    accptRate = 1E2_dp * self%outaccpt/self%outatmps
    write(nout,"(1x,A,F15.8)") "EB_AVBMC In Acceptance Rate: ", accptRate
