...
!===============================
  subroutine BCC(BoxNum, latConst, replicate)
    ...
    real(dp) :: boxL
    real(dp) :: baseCell(1:2, 1:3)

    write(nout,*) "Box", boxNum, "Creating BCC Structure with parameters:", latConst
!    allocate( OrthoBox::BoxArray(BoxNum)%box )
    ...
    baseCell(2, 3) = 0.5*latConst


    boxL = replicate * latConst
    nAtoms = 2*replicate**3
