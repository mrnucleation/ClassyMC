!===================================================================
! Easy Pair Template for LJ Cut and Shift pair forcefields. 
!===================================================================
module FF_EasyEP_LJ_CutShift
  use FF_EasyPair_Cut, only: EasyPair_Cut
  use VarPrecision
  use Template_SimBox, only: SimBox
  use CoordinateTypes

  type, extends(EasyPair_Cut) :: EP_LJ_CutShift
...
!    real(dp) :: rCut, rCutSq
    ...
    contains
      ...
      procedure, pass :: CountType => CountType_EP_LJ_CutShift
      procedure, pass :: SumTail => SumTail_EP_LJ_CutShift
  end type

  contains
  !=============================================================================+
  subroutine Constructor_EP_LJ_CutShift(self)
  ...
  !=============================================================================+
  function PairFunction_EP_LJ_CutShift(self, rsq, atmtype1, atmtype2) result(E_Pair)
    use Common_MolInfo, only: nAtomTypes
    implicit none
    class(EP_LJ_CutShift), intent(inout) :: self
  ...
  function SumTail_EP_LJ_CutShift(self) result(E_Corr)
    use Common_MolInfo, only: nAtomTypes
  ...
  ! simulation box
  subroutine CountType_EP_LJ_CutShift(self, NMol) 
  ...
  !======================================================================+
   subroutine TailCorrection_EP_LJ_CutShift(self, curbox, E_Corr, disp) 
  ...
  subroutine ComputeTypeCount_EP_LJ_CutShift(self)
  ...
  !=====================================================================
  subroutine ProcessIO_EP_LJ_CutShift(self, line)
    use Common_MolInfo, only: nAtomTypes
      ...
      select case(nPar)
        case(4)
          ...
          do jType = 1, nAtomTypes
            ...
            self%shiftTable(type1, jType) = self%PairFunction(self%rCutSq, type1, jType)
            self%shiftTable(jType, type1) = self%shiftTable(type1, jType)
          enddo
        case(5)
          ...
          self%shiftTable(type1, type2) = self%PairFunction(self%rCutSq, type1, type2)
          self%shiftTable(type2, type1) = self%shiftTable(type1, type2)
